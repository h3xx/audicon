function [o]=foreach_split(wavfile, fcns, clip_duration, tex_span, step)
...
// Part of project Audicon by Dan Church <daniel.church@uwrf.edu>
	...
	for a = [ 'fcns' 'clip_duration' 'tex_span' 'step' ]
		if ~exists(a, 'local')
	...
	for i = 1 : jump : num_samples
		...
		prev_clip = buff(prev_ind : (prev_ind + clip_size));
		curr_clip = buff(curr_ind : (curr_ind + clip_size));
		next_clip = buff(next_ind : (next_ind + clip_size));
		prev_tex = buff(prev_tex_ind : (prev_tex_ind + ..
				sum(tex_size)));
		curr_tex = buff(curr_tex_ind : (curr_tex_ind + ..
				sum(tex_size)));
		next_tex = buff(next_tex_ind : (next_tex_ind + ..
				sum(tex_size)));
		
		// run each function
		o = [ o ; calc_features(fcns, sampling_rate, curr_clip, ..
			curr_tex, prev_clip, prev_tex, next_clip, next_tex) ];

		// DEBUG: save calculated vars
		save(wavfile + '-vars', o);
		// END DEBUG
